...
feature 'TrueAutomation.IO capybara example' do
  ...
  scenario 'Recording - Create account' do
    ...
    visit 'https://annieselke.com/'
      ...
      find(:xpath, ta('AS:mainPage:loginDesktop', "//a[text()='Login']")).click
      # User clicks on create account for starting greating new account
      find(:xpath, ta('AS:mainPage:createAccount', "//h3[text()='Create an Account']")).click
      # User set first name in matching input line
      find(:xpath, ta('AS:loginPage:firstName', "//input[@id='register.firstName']")).set('youre.example@gmail.com')
      # User set last name in matching input line
      find(:xpath, ta('AS:loginPage:lastName', "//input[@id='register.lastName']")).set('name')
      # User set email in matching input line
      find(:xpath, ta('AS:loginPage:email', "//input[@id='register.email']")).set('name')
      # User set pass in matching input line
      find(:xpath, ta('AS:loginPage:password', "//input[@id='password']")).set('1234tesT')
      # User repeat pass name in matching input line
      find(:xpath, ta('AS:loginPage:checkPwd', "//input[@id='register.checkPwd']")).set('1234tesT')
      # User click in checkbox for subscribe
      find(:xpath, ta('AS:loginPage:subscribe', "//label[@for='subscribe']")).click
      # User click register button to complete the registration
      find(:xpath, ta('AS:loginPage:register', "(//button[@class='primary actionSet'])[2]")).click
